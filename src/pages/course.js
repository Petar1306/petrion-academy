...
async function render() {
  ...
  try {
    ...
    console.error(err);
    contentEl.innerHTML = '<div class="alert alert-danger">Course not found.</div>';
    return;
  }
  ...
  if (!isLoggedIn) {
    ...
    actionArea = `<button id="enrollBtn" class="btn btn-petrion">
        <i class="bi bi-mortarboard me-1"></i> Enroll in this course
      </button>`;
  }
    ...
    enrollBtn.addEventListener('click', async () => {
      ...
      try {
        ...
        document.getElementById('actionArea').innerHTML =
          `<div>Your status: ${statusBadge.pending}</div>`;
        document.getElementById('enrollMessage').innerHTML =
          '<div class="alert alert-success py-2">Enrollment request sent! Your teacher will review it.</div>';
      } catch (err) {
        console.error(err);
        document.getElementById('enrollMessage').innerHTML =
